...
class SdkMcpServerTest {
    ...
    @Test
    void annotatedMethodRegistration() throws Exception {
        // Define tool class in a separate file would be cleaner; for in-test
        // simplicity we just verify the (Object, Method) overload compiles.
        var server = SdkMcpServer.builder("math", "1.0")
            .tool("double", "Doubles a number", Integer.class,
                args -> ToolResult.text(String.valueOf(((Integer) args) * 2)))
            .build();
        var tool = server.findTool("double");
        assertThat(tool).isNotNull();
        ToolResult result = tool.handler().apply(21);
        assertThat(result.content().get(0).get("text")).isEqualTo("42");
    }
}
